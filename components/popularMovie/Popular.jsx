...
const Popular = ({ popularMoviesList }) => {
  const [current, setCurrent] = useState(0);
  const length = popularMoviesList.length;
  ...
  if (!Array.isArray(popularMoviesList) || popularMoviesList.length <= 0) {
    return null;
  }
  ...
  return (
    <div className="text-white">
      ...
      <div className="relative w-full h-[75vh] flex justify-center p-4 my-10">
        {popularMoviesList.map((slide, index) => {
          return (
            <div
              key={index}
              className={
      ...
      <div className="flex flex-wrap gap-y-5 items-center justify-center gap-x-2 py-1 mb-10 top-4">
        {popularMoviesList.map((slides, slidesId) => (
          <div key={slidesId}>
            <RxDotFilled
